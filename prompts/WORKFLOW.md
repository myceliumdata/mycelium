...
1. Look in `prompts/next/`
2. Select the oldest task (sorted by filename)
3. Claim it by moving the file to `prompts/in-progress/`
4. Execute the work
5. Deliver results under `prompts/done/`
...
A simple test task currently exists in `prompts/next/` (`2025-06-01-1650-test-workflow.md`) that you can use to validate the full flow before running real work.
...
- Grok creates a new file in `prompts/next/` named:
...
- The prompt must include:
  ...
  - Required output format and location (`prompts/done/<name>/`)
...
1. Scan the directory `prompts/next/`.
2. List all `.md` files.
...
5. **Immediately move** that file from `prompts/next/` to `prompts/in-progress/`.
...
**Rules for Cursor:**
- Never work on a file that is still in `prompts/next/`. Always claim it first by moving it.
- If `prompts/next/` is empty, clearly report that there is no work available.
...
1. It creates a directory under `prompts/done/` with the same base name as the prompt.
...
4. **It must remove only the specific file it claimed** from `prompts/in-progress/`.
...
- Automatically discover the oldest task in `prompts/next/`
- Claim it by moving it to `prompts/in-progress/`
- Execute it according to the instructions inside the prompt
- Deliver output under `prompts/done/`

You should no longer need to specify filenames when starting work.

## Prompt Template Requirements

Every prompt in `prompts/next/` **must** contain:
...
- All completed work is archived under `prompts/done/<prompt-name>/`.
...
**Simple rule:** Any directory inside `prompts/done/` that does **not** contain a `review.md` file is considered "pending review."

To see what needs attention:
- Look for folders under `prompts/done/` without a `review.md`
- Or run a quick command like: `ls prompts/done/*/review.md 2>/dev/null || echo "Nothing reviewed yet"`
...
- If changes are needed, create a new prompt in `prompts/next/`
...
- Create a follow-up prompt in `prompts/next/` describing what needs to be done instead.
