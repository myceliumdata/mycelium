# Task: Reprocess - Redesign Slice 1540: specialist-template-base (from 2026-06-07 redesign)
...
**Objective**
Re-execute exactly the 1540-specialist-template-base slice as described in the 'Completed & Reviewed Slices' section of the redesign_reset.md for **1540-specialist-template-base** (reviewed).
...
- Only implement what was in the 1540 slice; do not do later slices' work.
- Run only smoke tests + the specific verifs from the reset for this slice.
- When delivering, put into done/2026-06-09-1540-specialist-template-base-reprocess/ (new name) with prompt.md (copy of this), output.md.
- Deliver only prompt.md and output.md. Do not create review.md — Grok will review the work and add the review.md after.

**Context**
The full history, what the original prompt contained, what was delivered, the verifications that passed, and the state before/after this slice are all in prompts/resets/2026-06-07_redesign_reset.md under the 1540 section and the "Current Project State Snapshot".
...
1. **Claim the task first (mandatory per WORKFLOW.md)**: Move this file to `prompts/cursor/in-progress/2026-06-09-1540-specialist-template-base-reprocess/prompt.md`. Document in your output.md.
2. **Discovery (read-only)**: Read the 1540 section in redesign_reset.md , current specialist_agent.py.j2, base.py, agent_factory.py, test_agent_factory.py, etc.
3. **Implement the changes for this slice exactly**: Perform the actions described for 1540 in the reset: Updated specialist_agent.py.j2 (header, removed core_identity, added person_id/context/target_fields resolution, full 3 scenarios with _start_research_if_needed + daemon thread stub + pending/na/found logic + specialist_contrib, robustness TODO exact quote, uses unified builders with base_records). base.py: comment on person_id keys in records. agent_factory.py: refine prompt updated. test_agent_factory.py minimal marker updates. Smoke green. Manual tmp factory create + dynamic load/invoke test exercises scenarios correctly (pending path writes status, returns correct contrib/message).
4. **Verification**: Run the tests green as listed for 1540 in the reset. Update any affected tests/fixtures as in original.
5. **Output artifacts**: Create the done/ dir with new name 2026-06-09-1540-specialist-template-base-reprocess/ containing:
   - prompt.md (this file)
   - output.md with summary, git diff --stat, full verification outputs, scope confirmation, "Ready for next slice: 2026-06-09-1550-supervisor-context-graph-reprocess.md".
   (Grok will add review.md after reviewing the delivered prompt.md + output.md.)
6. Remove only your claim from in-progress/.

**Scope Boundaries**
Follow exactly the scope from the original 1540 as described in the redesign_reset.md 1540 section. Do not touch things for later slices (e.g. do not implement the supervisor context graph nodes or later changes yet).

**Test Execution Policy**
Default smoke, plus the specific for this slice.

**Required Output Location & Artifacts**
prompts/cursor/done/2026-06-09-1540-specialist-template-base-reprocess/ (use the reprocess name so it does not overwrite any previous version in done/).

Follow WORKFLOW.md claiming exactly.

This reprocess is to restore the processed Cursor task whose output was destroyed in a previous backout. Make the implementation and artifacts match what was reviewed and approved in the original 1540 as documented in the redesign_reset.md.
