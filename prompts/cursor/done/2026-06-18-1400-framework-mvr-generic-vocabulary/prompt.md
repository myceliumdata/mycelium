...
| G15 | **Real MVR field names when specific** — `bind_values`, `lookup`, `results[]`, and field-level messages use manifest keys (`employer`, `team`, …). No `bind_field_1` or positional labels. Say “employer” on CRM because CRM MVR defines `employer`; say “team” on baseball. Generic “bind fields” / “MVR” only when describing the mechanism, not when naming a field. |
