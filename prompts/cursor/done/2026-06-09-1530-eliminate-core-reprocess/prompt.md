# Task: Reprocess - Redesign Slice 1530: eliminate-core (from 2026-06-07 redesign)
...
**Objective**
Re-execute exactly the 1530-eliminate-core slice as described in the 'Completed & Reviewed Slices' section of the redesign_reset.md for **1530-eliminate-core** (reviewed).
...
- Only implement what was in the 1530 slice; do not do later slices' work.
- Run only smoke tests + the specific verifs from the reset for this slice.
- When delivering, put into done/2026-06-09-1530-eliminate-core-reprocess/ (new name) with prompt.md (copy of this), output.md.
- Deliver only prompt.md and output.md. Do not create review.md — Grok will review the work and add the review.md after.

**Context**
The full history, what the original prompt contained, what was delivered, the verifications that passed, and the state before/after this slice are all in prompts/resets/2026-06-07_redesign_reset.md under the 1530 section and the "Current Project State Snapshot".
...
1. **Claim the task first (mandatory per WORKFLOW.md)**: Move this file to `prompts/cursor/in-progress/2026-06-09-1530-eliminate-core-reprocess/prompt.md`. Document in your output.md.
2. **Discovery (read-only)**: Read the 1530 section in redesign_reset.md , current core_data.py, dispatch.py, registry.py, supervisor.py, etc.
3. **Implement the changes for this slice exactly**: Perform the actions described for 1530 in the reset: core_data.py deleted. dispatch.py: no core fallback, raises on unknown route. registry.py + data/agent_registry.json: no core_data entry (seed now empty agents). supervisor.py: seed resolution via find_by_key, direct responses from seed when no specialists needed, still plans/creates real specialists. __init__.py, bin/reset-mycelium, tests updated (test_core_data_agent.py now just skip). Graph clean. Queries now use seed path. reset-mycelium dry-run works. No core_data in runtime.
4. **Verification**: Run the tests green as listed for 1530 in the reset. Update any affected tests/fixtures as in original.
5. **Output artifacts**: Create the done/ dir with new name 2026-06-09-1530-eliminate-core-reprocess/ containing:
   - prompt.md (this file)
   - output.md with summary, git diff --stat, full verification outputs, scope confirmation, "Ready for next slice: 2026-06-09-1540-specialist-template-base-reprocess.md".
   (Grok will add review.md after reviewing the delivered prompt.md + output.md.)
6. Remove only your claim from in-progress/.

**Scope Boundaries**
Follow exactly the scope from the original 1530 as described in the redesign_reset.md 1530 section. Do not touch things for later slices (e.g. do not implement specialist template changes, context builder, or later yet).

**Test Execution Policy**
Default smoke, plus the specific for this slice.

**Required Output Location & Artifacts**
prompts/cursor/done/2026-06-09-1530-eliminate-core-reprocess/ (use the reprocess name so it does not overwrite any previous version in done/).

Follow WORKFLOW.md claiming exactly.

This reprocess is to restore the processed Cursor task whose output was destroyed in a previous backout. Make the implementation and artifacts match what was reviewed and approved in the original 1530 as documented in the redesign_reset.md.
