...
- [ ] **Review derivative stats under test** — Audit everything the baseball program gates today: manifest recipes vs LLM derive vs product specialists. Sources: [`tests/live/catalogs/baseball.yaml`](tests/live/catalogs/baseball.yaml), anchors [`tests/live/anchors/baseball_aaron_lahman_v2025.json`](tests/live/anchors/baseball_aaron_lahman_v2025.json), [`docs/manual-checks/2026-06-19-baseball-specialist-hand-test.md`](docs/manual-checks/2026-06-19-baseball-specialist-hand-test.md). Confirm each stat is the right *kind* of derivative (warehouse convention, derive-on-miss, cross-table product) and anchors match the intended table/convention (e.g. Fielding vs Batting `G`). Outcome: short table in conversation or hand-test doc; fix anchors/catalog before expanding derive domains.
- [ ] **Confirm training wheels off for derivative data** — M4 removed `derive_candidates` whitelist; batting has `derive_on_miss: true`. Verify no remaining caps: domain flags (pitching/fielding still off), semantic-review bypass env, sandbox loosening, intent-map shortcuts, gate `fresh_derive_before_gate` masking stale cache bugs. Reference: [`docs/plans/conversations/2026-06-19-baseball-m4-free-form-derive.md`](docs/plans/conversations/2026-06-19-baseball-m4-free-form-derive.md). Paul sign-off that “any manifest miss → derive” is production policy on enabled domains.
- [ ] **Baseball bio specialist — design lock before implement** — **Morning brief:** [`docs/plans/conversations/2026-06-21-baseball-morning-decision-brief.md`](docs/plans/conversations/2026-06-21-baseball-morning-decision-brief.md) — **Part H** (review settled) + **Part D** (bio Q1–Q8 still needed for `2410`). Orchestration/`race` locked separately. Draft prompt: `prompts/cursor/next/2026-06-21-2410-baseball-bio-research-specialist.md`.
