...
- [ ] **Baseball pack → framework extraction review** — **Depth pass** under [Example networks → framework promotion audit](#examples--demos-open) (baseball first). Audit remaining pack code (`warehouse_resolve`, `derive_resolve`, `product_common`, bootstrap handlers) for promotion into `src/`. M14 started hierarchy; finish resolver protocols + `ProductTeamSpecialist`. **Rename review:** M14 `WarehousePlayerStatSpecialist` / `WarehouseTeamStatSpecialist` use network-flavored “Player”/“Team” — consider grain-agnostic framework names (Paul Q1, 2026-06-21); new tier is `WarehouseResearchStatSpecialist`.
...
- [ ] **Example networks → framework promotion audit** — Review **all** committed examples (`crm-seeded`, `crm-empty`, `crm-metering`, `baseball`) for code/patterns that should live in `src/` vs stay in pack (`examples/networks/<name>/`): bootstrap handlers, warehouse/derive/product resolvers, specialist subclasses, record-type routing, metering negotiation, research/derive policy flags, shared test fixtures. **Outcome:** promotion table per network (promote / keep pack / defer) + prioritized Cursor slices. Hierarchy rationale: [`docs/architecture/whys/specialist-class-hierarchy.md`](docs/architecture/whys/specialist-class-hierarchy.md). **Baseball** is the richest candidate — see **Next up** item above for the first deep pass.
...
- [ ] **Ontology hand-add vs self-creating network** — Paul sign-off 2026-06-21: hand-add gate attrs for `2410`; **review** that committed `attribute_map` + lazy classification growth (CRM) stay aligned with self-creating data network goal. Related: bio `primary_nickname` / `hall_of_fame_year` entries.
...
- [ ] **Manifest derivative promotion (derive → warehouse alias)** — Design how LLM derive-on-miss stats **graduate** to pre-defined manifest aliases in `warehouse_domains.json` (lighter than full specialist promotion). Baseball proof: `career_avg` and `ops` *could* be manifest today but were left unaliased to gate-test codegen (`bb-derive-01`–`03`). Open questions: promotion signals (hit count, formula stability across semantic reviews, storage vs re-derive cost), who initiates (operator, agent recommendation, automated threshold), mapping `intent_slug` → convention + column recipe, provenance migration (derive inline → resolver function), gate/anchor updates, demotion/rollback. **Document** the manifest-vs-derive split in `docs/architecture/whys/manifest-derivatives.md`. Paul + Grok 2026-06-21.
- [ ] **Peer-aware specialists + analytic orchestration** — Design lock: [`docs/plans/conversations/2026-06-21-peer-aware-specialists-analytic-orchestration.md`](docs/plans/conversations/2026-06-21-peer-aware-specialists-analytic-orchestration.md). Product/analytic specialists get ontology peer map + full warehouse briefing; **delegate** bio-owned facts (e.g. `race`) via dispatch — bio uses Tavily, not product. Analytic specialist **discovers cohort** from warehouse (supervisor does not enumerate players). Framework: specialist briefing, peer read/fill protocol, `ProductTeamSpecialist` orchestration hooks. Example: `best_batting_group_by_race` + 1985 team scope. Depends on bio research (`2410`); add `race` to bio ontology after bio path proven.
- [ ] **Metering — orchestrated multi-specialist quotes** — Review pricing from analytic orchestration perspective: variable cohort size, N×bio research vs cache hits, aggregate compute line items. Paul lock: no technical research cap — **client pays** when quote is correct. Open: quote after cohort discovery vs estimate + reconcile; entitlement on cache-heavy redelivers. [`2026-06-21-peer-aware-specialists-analytic-orchestration.md`](docs/plans/conversations/2026-06-21-peer-aware-specialists-analytic-orchestration.md) § Metering. Related: [`docs/architecture/whys/metering-economics.md`](docs/architecture/whys/metering-economics.md).
...
- [ ] **Specialist promotion (derive → product specialist)** — **Out of scope for automation now.** Heavier tier after **manifest derivative promotion** above: derive telemetry + cost signals → agent recommends new category + pack specialist for repeat cross-domain computations; Paul/Grok approve slices until automated. Suppress factory research stub for warehouse categories on baseball. [`2026-06-19-warehouse-factory-layer3-specialist-emergence.md`](docs/plans/conversations/2026-06-19-warehouse-factory-layer3-specialist-emergence.md).
...
Last updated: 2026-06-22 (example → framework promotion audit on roadmap)