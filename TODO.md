...
0. [ ] **Multi-match step-2 deliver truncates + wrong message (bug)** — Paul/MCP: step 1 `lookup: {name: Andrea Kalmans}` + attrs → `total_matches: 2`; step 2 returns **`results` length 1** (not 2). **Reproduced locally** (seed Andrea @ Lontra `validated` + bind Andrea @ Wrong Corp `provisional`). **Cause:** one provisional in batch → `research_gate_allows` fails for whole scope → `assemble_response` calls `response_research_gated(..., matched[0])` only (M8 batch deliver bypassed). **Also wrong:** message says “Found **provisional** record … at Lontra Ventures” even when `matched[0]` is **validated** — template is batch-level gate, single-record message. **Expected (M8 / `multi_match` policy):** `results[]` has **all N** identity rows; research per validated row or per-record gate status; agent can disambiguate. Tied to item 1 (Wrong Corp never promoted). Queue Cursor slice(s) — may merge with item 1.
1. [ ] **Provisional validation on step-2 deliver (bug)** — Registry rows from create-on-deliver / query bind stay **`provisional`** forever when core validation never runs. Observed: Andrea @ Wrong Corp after `confirm_new_entity`. **Gaps:** (a) step-2 **identity-only** deliver short-circuits in `target_resolve_node` → `validate_entity` never runs; (b) step-2 with **`len(matched) > 1`** skips `validate_entity`. **Fix:** ensure promotion on applicable step-2 paths; smoke tests. See item 0 for multi-match deliver symptom.
