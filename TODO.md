...
0. [ ] **Fuzzy employer partial lookup** — Cursor [`1435`](prompts/cursor/next/2026-06-14-1435-partial-lookup-fuzzy-employer-suggestions.md) after `1430` lands.
1. [ ] **Program 2 manual gate (fresh from scratch)** — Follow [`docs/manual-checks/2026-06-13-program2-post-program-gate.md`](docs/manual-checks/2026-06-13-program2-post-program-gate.md). Update gate table row: partial name typo → `lookup_suggested`. Branch ahead of `origin/main` — run `git log origin/main..HEAD --oneline | wc -l` before testing; no push until gate **CLEAR**.
...
- [ ] **Fuzzy lookup (bind fields)** — Near-miss → `lookup_suggested` on 0 exact index hits for **every** `mvr.bind_fields` entry (not only `name`). Policy: [`docs/plans/fuzzy-lookup-policy.md`](docs/plans/fuzzy-lookup-policy.md). **Name:** slice [`1430`](prompts/cursor/done/2026-06-14-1430-partial-lookup-fuzzy-name-suggestions/) (review/land). **Employer:** slice [`1435`](prompts/cursor/next/2026-06-14-1435-partial-lookup-fuzzy-employer-suggestions.md) queued. v1 algorithm: `SequenceMatcher` @ `SUGGESTION_MIN_SCORE` (0.85).
- [ ] **Fuzzy match upgrades (aliases & prefixes)** — Current fuzzy handles **typos** (e.g. `Andrea Kalman`, `654 Ventures` → `645 Ventures`), not **shorthand** (`645` → 645 Ventures) or **prefix nicknames** (`ibm` → IBM Corporation). Follow-up: prefix index, alias table, or token-overlap — design with search-indices work.
- [ ] **Query / search any field (future)** — Product requirement: clients can query on **any** attribute field, not only MVR/bind/indexed fields. v1 MVR redesign: lookup on indexed MVR subset + UUID identity; this item tracks general-field search (extended attrs, specialist storage scan or secondary indices, optional query restrictions per network). **Include fuzzy suggestions** on 0-hit any-field search (same policy as bind fields). Deferred until after MVR redesign + index story matures.
