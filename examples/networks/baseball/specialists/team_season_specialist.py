...
def _resolve_owned_fields(state: MyceliumGraphState) -> list[str]:
    ...
    for cl in state.classifications or []:
        if cl.get("category") == "team_season" and cl.get("attribute"):
...
def _evaluate_team_season_fields(
...
def _run_team_season_specialist_graph(state: MyceliumGraphState | dict[str, Any]) -> dict[str, Any]:
    current = _coerce(state)
    ctx = current.context if isinstance(state.context, dict) else {}
    tid = _resolve_id(current)
    ...
    identity_records = _identity_from_context(ctx, tid)

    if not tid:
        resp = response_not_found(current.query, specialist="team_season_specialist", **id_kwargs, **clf_kwargs)
        return {
            "response": resp,
            "route": None,
            "audit_log": ["team_season_specialist: no id in state."],
    ...
    values, overall_status = _evaluate_team_season_fields(AGENT, tid, owned, paths=paths)
    contrib = {
        "id": tid,
    ...
    return {
        ...
        "route": None,
        "audit_log": [
            f"team_season_specialist: {overall_status} for id={tid!r} (category=team_season).",
        ],
...
class TeamSeasonSpecialist(SpecialistAgent):
    ...
    def run(self, state: MyceliumGraphState | dict[str, Any]) -> dict[str, Any]:
        return _run_team_season_specialist_graph(state)
