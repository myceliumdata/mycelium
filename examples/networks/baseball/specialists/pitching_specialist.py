"""pitching_specialist — warehouse-backed career pitching stats (baseball pack)."""
...
def _resolve_owned_fields(state: MyceliumGraphState) -> list[str]:
    ...
    for cl in state.classifications or []:
        if cl.get("category") == "pitching" and cl.get("attribute"):
...
def _run_pitching_specialist_graph(state: MyceliumGraphState | dict[str, Any]) -> dict[str, Any]:
    ...
    if not pid:
        resp = response_not_found(current.query, specialist="pitching_specialist", **id_kwargs, **clf_kwargs)
        return {
            "response": resp,
            "route": None,
            "audit_log": ["pitching_specialist: no id in state."],
    ...
    values, overall_status = _evaluate_pitching_fields(AGENT, pid, owned, paths=paths)
    contrib = {
        "id": pid,
        "category": "pitching",
        "fields": owned,
    ...
    return {
        ...
        "route": None,
        "audit_log": [
            f"pitching_specialist: {overall_status} for id={pid!r} (category=pitching).",
        ],
...
class PitchingSpecialist(SpecialistAgent):
    category = "pitching"
    agent_name = "pitching_specialist"

    def run(self, state: MyceliumGraphState | dict[str, Any]) -> dict[str, Any]:
        return _run_pitching_specialist_graph(state)


AGENT = PitchingSpecialist()


def pitching_specialist(state: MyceliumGraphState | dict[str, Any]) -> dict[str, Any]:
