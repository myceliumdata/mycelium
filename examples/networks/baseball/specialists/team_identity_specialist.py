...
def _resolve_owned_fields(state: MyceliumGraphState) -> list[str]:
    ...
    for cl in state.classifications or []:
        if cl.get("category") == "team_identity" and cl.get("attribute"):
...
def _bind_values_for_fields(
    ...
    if missing:
        entity = get_entity_registry(record_type="team").lookup_by_id(entity_id)
        if entity is not None:
            for key in missing:
                value = entity.bind_value(key)
                if value is not None:
                    out[key] = value
    allowed = frozenset(load_mvr(record_type="team").bind_fields)
...
def _evaluate_team_identity_fields(
    agent: SpecialistAgent,
    entity_id: str,
    owned: list[str],
    *,
...
def _run_team_identity_specialist_graph(
    ...
    if not tid:
        resp = response_not_found(
            current.query,
            specialist="team_identity_specialist",
            **id_kwargs,
            **clf_kwargs,
        )
        return {
            "response": resp,
            "route": None,
            "audit_log": ["team_identity_specialist: no id in state."],
            "specialist_contrib": {
                "id": None,
                "fields": owned,
                "values": {},
    ...
    values, overall_status = _evaluate_team_identity_fields(
        AGENT,
        tid,
        owned,
        ctx=ctx,
    ...
    return {
        ...
        "route": None,
        "audit_log": [
            f"team_identity_specialist: {overall_status} for id={tid!r} "
            "(category=team_identity).",
        ],
...
class TeamIdentitySpecialist(SpecialistAgent):
    ...
    def run(self, state: MyceliumGraphState | dict[str, Any]) -> dict[str, Any]:
        return _run_team_identity_specialist_graph(state)
