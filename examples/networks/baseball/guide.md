...
- **Player** identity uses `player` (display name) + `team` for bind disambiguation (any team the player appeared for may alias the same player via bind aliases at bootstrap).

**Query keys:** `{player, team}` routes to player grain; `{team}` routes to team grain. `{player}` alone resolves when the name hits the registry (unique or homonym multi-match); unknown names return `lookup_incomplete` asking for `team`. No `grain` override on step 1.
