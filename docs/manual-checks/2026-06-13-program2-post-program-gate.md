...
|-------|---------|-------------|
| **Committed runtime in example tree** | `refresh` copied `examples/networks/crm-seeded/entities.json` → seed import saw duplicates → **Entities ✅ but specialists ❌** | Example tree must ship **reference files only** (`seed.json`, `network.json`, `guide.md`, README). Layout tests fail if `entities.json`, `deliveries.json`, or `agents/` exist under `examples/networks/`. **Not gitignored** — stray files show in `git status`. |
...
| **crm-empty step 2 MVR bootstrap** | Step 1 OK; step 2 → `not_found` / **"No valid delivery"** (misleading — real error was missing `attribute_map` for `name`/`employer`) | `target_deliver` calls `ensure_categories_for_mvr_bind` before bind write on create-on-deliver. **Check 4b** is the hands-on proof. |
...
- If `entities.json` reappeared under `examples/networks/crm-seeded/`, delete it — something wrote runtime data into the example tree
...
| crm-empty create-on-deliver | `test_crm_empty_refresh_capstone_create_on_deliver_storage` |
...
| Item | Note |
|------|------|
| Network roots | `~/mycelium-networks/crm-seeded` and `~/mycelium-networks/crm-empty` (or your paths) |
...
Run checks **0 → 0b → 0c → 4b → 1 → 2 → 3 → 4 → 6 → 7**. Check **0c** (lookup clarity) needs seeded `crm`. Check **4b** before **4** because crm-empty is the cold-start path; crm Road Runner in Check 4 assumes seeded network. Check 5 is **automated only** (skip manual).

Set once:

```bash
CRM_ROOT=~/mycelium-networks/crm-seeded
EMPTY_ROOT=~/mycelium-networks/crm-empty
```
...
|---------|----------|-------------|-----------|
| `crm-empty` | ❌ | ❌ | No seed import |
| `crm` | ✅ (15) | `demographic (15)` + `professional (15)` | Refresh printed `seed: … → 15 entities imported` |

**Automated:** `test_matrix_a_crm_refresh_seed_bootstrap_storage` + `test_matrix_b_crm_empty_refresh_create_on_deliver_bind`
...
## Check 4b — crm-empty Paul Murphy create-on-deliver (required)
...
uv run mycelium query --network crm-empty --delivery-id <delivery_id>
```
...
- `uv run mycelium network status --network crm-empty` → Entities ✅ (1)
...
**Automated:** `test_crm_empty_refresh_capstone_create_on_deliver_storage`, `test_crm_empty_safe_create_without_confirm`
...
uv run mycelium query --network crm-seeded --delivery-id <delivery_id>
```
...
|---------|--------------|--------|
| `entities.json` under `examples/networks/crm-seeded/` | Runtime leaked into example tree | Delete file; fix whatever wrote it; re-run pre-flight A |
...
| **crm-empty has entities/specialists after refresh only** | Should never happen (no seed) | Re-refresh crm-empty; check you are not pointing at `crm` root |
| Step 2 **"No valid delivery"** on crm-empty | MVR bootstrap regression (pre-fix) or stale code | Confirm remedial slices on branch; re-run **4b** |
...
*Created: 2026-06-13 · Rewritten: 2026-06-14 (seed-refresh, crm-empty MVR, capstone tests) · Updated: 2026-06-14 (step-1 lookup clarity, fuzzy suggestions 1430–1450, `suggested_lookup`) · **CLEAR** 2026-06-14*