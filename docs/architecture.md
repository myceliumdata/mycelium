...
- **Bootstrap phase:** `network.bootstrap.run_network_bootstrap(paths)` orchestrates create/refresh bootstrap — applies paths, MVR category merge, registry reset, loads `guide.md` into `BootstrapContext`, then invokes a bootstrap handler. Default handler imports `seed.json` via `ensure_bound_entity` (`source=seed_bootstrap`, `validation_state=validated`). `bootstrap_seed_at_paths()` is a thin wrapper returning `entities_committed`. Networks may override with `<network_root>/specialists/bootstrap_specialist.py` exposing `run_bootstrap(ctx)`. Bypasses the two-step lookup/create protocol by design; baseball cold start will extend `network/bootstrap/handlers/` rather than forking refresh.
