...
- **Runtime store:** Per-grain entity files at `<network_root>/entities/<grain>.json`. Each file holds uuid4 ids, `bind_values` keyed by that grain's `mvr.grains.<grain>.bind_fields`, generic `bind_index`, and per-field indexes. `ensure_entity_bind_fields` assigns stable ids on import; step-1 resolve **fans out** lookups per declared grain (or single-grain path for CRM) — see [query-grain-router.md](query-grain-router.md). Seed `rows[]` import into the bootstrap grain (`bootstrap.seed_grain` or `mvr.default_grain`) on refresh or `network create`.
