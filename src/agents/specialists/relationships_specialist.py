...
AUTO-GENERATED by Agent Factory on 2026-06-04T08:38:02.735966+00:00.
...
def relationships_specialist(state: MyceliumGraphState | dict[str, Any]) -> dict[str, Any]:
    """
    Specialist agent for the "relationships" category.
    ...
    for c in (current.classifications or []):
        if c.get("category") == "relationships" and c.get("attribute"):
    ...
    if not matches:
        ...
        resp = response_non_core(
            current.query,
            matches,
            my_attrs or ["relationships data"],
    ...
    logs = [
        f"relationships_specialist: lookup {outcome} for person_key={current.query.person_key!r} (category=relationships).",
