...
Every CLI and MCP query response includes **`outcome`** (machine-readable: `lookup_resolved`, `lookup_incomplete`, `lookup_suggested`, `quote_required`, `payment_required`, `principal_required`, `found`, `assembled`, `not_found`, or `error`). Step 1 returns `delivery` + `total_matches` when resolved; when step 2 will create a new entity, `delivery.create_on_deliver` is `true` (omitted for existing matches). Step 1 may instead return `lookup_incomplete` (`required_fields`) or `lookup_suggested` (`suggestions[].suggested_lookup` retry maps — e.g. `{"employer":"645 Ventures"}` after `645 Venture`). Step 2 returns full `results[]`. Agents should branch on `outcome` and `delivery.create_on_deliver` before trusting `results`; use `message` for step-1 deliver guidance and per-attribute status. When metering payment is enabled: `quote_required` → `pay_quote` → step 2 with `quote_id`. MCP schema: `mycelium://schema/query-response`. Examples: [`docs/plans/mvr-redesign-entity-query-examples.md`](docs/plans/mvr-redesign-entity-query-examples.md); fuzzy policy: [`docs/plans/fuzzy-lookup-policy.md`](docs/plans/fuzzy-lookup-policy.md).

- **`suggestions`** — On `lookup_suggested` only. Each entry has `reason`, optional display fields, and **`suggested_lookup`** (partial MVR map to merge into step-1 `lookup`) or `id` for a known row.
- **`required_fields`** — On `lookup_incomplete` only. Missing MVR bind keys (e.g. `["employer"]` when only `name` was sent).
...
flowchart TD
    ...
    Entities[(network_root/entities/person.json)] --> TR
