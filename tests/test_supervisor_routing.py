...
def test_routing_delegates_lookup_to_identity_accessor() -> None:
    person = Person(id="p1", name="Ada", employer="Lab")
    identity = _StubIdentity(person)
    state = MyceliumGraphState(query=PersonQuery(person_key="Ada"))

    decision = evaluate_supervisor_turn(state, identity=identity)
...
def test_routing_persist_after_validation() -> None:
    ...
    assert decision.action == "respond"
    assert identity.persisted == [person]
