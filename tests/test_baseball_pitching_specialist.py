...
def _deliver_career_wins(*, provenance: bool = False) -> tuple[object, object]:
    step1 = EntityQuery(
        lookup=dict(SAMPLE_PLAYER),
        requested_attributes=["career_wins"],
        provenance=provenance,
    )
    r1 = run_query(step1, thread_id="career-wins-step1")
    ...
    r2 = run_query(step2, thread_id="career-wins-step2")
    return r1, r2
...
def test_career_wins_compute_found(
    ...
    _, response = _deliver_career_wins()
...
def test_career_wins_provenance_shape(
    ...
    refresh_baseball_root(tmp_path, monkeypatch)
    _, response = _deliver_career_wins(provenance=True)
